...
var sendMailNotification = function(t,cb ){
	// blat.exe -to jeffrey.sun@autodesk.com -server shacng6120067 -f 3dtrans@cloudservice.com
	// 	-subject "Your file translation is completed" -body "DONE."
    ...
    grep  = spawn('grep', ['ssh']);
	var blatName = __dirname + '/bin/windows/blat.exe';
	...
	var args = ['-to',
		...
		'shacng6120067',
		'-f', 
		'3dtrans@cloudservice.com', 
		'-subject', 
		subject,
		'-body',
		body];
	var blat = spawn(blatName, args);

	blat.stdout.on('data', function (data) {
	  logger.debug('stdout: ' + data);
	});

	blat.stderr.on('data', function (data) {
	  logger.debug('stderr: ' + data);
	});

	blat.on('exit', function (code, signal) {
		logger.debug('child process terminated with exit code '+code);
